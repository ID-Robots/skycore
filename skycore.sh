...
install_skycore() {
    ...
    echo -e "${GREEN}[✔]${NC} Created symlink at /usr/local/bin/skycore"
}
...
if [[ "$1" == "cli" ]]; then
    ...
    if [ -x "$TEST_RUNNER" ]; then
        echo -e "${YELLOW}[⋯]${NC} Running skycore test suite..."
        "$TEST_RUNNER"
    else
        echo -e "${RED}[✖]${NC} Test runner script not found at $TEST_RUNNER"
        echo -e "${YELLOW}[⋯]${NC} Make sure the tests directory exists and contains run_tests.sh"
        exit 1
    fi
