...
class RealsenseService:
    ...
    def find_device_by_serial(self, serial_number):
        """Finds a RealSense device by its serial number."""
        ctx = rs.context()
        devices = ctx.query_devices()
        for dev in devices:
            if dev.supports(rs.camera_info.serial_number) and dev.get_info(rs.camera_info.serial_number) == serial_number:
                ...
                if dev.supports(rs.camera_info.product_id) and product_id in self.DS5_product_ids:
                    logging.info(f"Found device with serial {serial_number} supporting advanced mode.")
                    return dev
                else:
                    logging.warning(f"Found device with serial {serial_number}, but it doesn't support advanced mode or isn't a D4xx series.")
                    return None # Or raise specific exception
        logging.error(f"Could not find device with serial number {serial_number}.")
        return None # Or raise specific exception

    def realsense_enable_advanced_mode(self, dev):
        """Enables advanced mode for a given device, handling reconnection."""
        ...
        while not advnc_mode.is_enabled():
            ...
            # We need to find the *same* device again, likely by serial number
            serial = dev.get_info(rs.camera_info.serial_number) # Get serial before it becomes invalid
            logging.info(f"Re-finding device with serial {serial} after enabling advanced mode.")
            dev = self.find_device_by_serial(serial)
            if not dev:
                raise RuntimeError(f"Failed to re-find device {serial} after enabling advanced mode.")
            advnc_mode = rs.rs400_advanced_mode(dev)
            logging.info("Advanced mode is %s" "enabled" if advnc_mode.is_enabled() else "disabled")
        return advnc_mode # Return the valid advanced mode object
    ...
    def realsense_load_settings_file(self, dev, setting_file):
        ...
        if os.path.isfile(setting_file):
            ...
            return # Don't exit, just skip loading
        ...
        dev.load_json(json_text)

    def realsense_connect(self):
        # Declare RealSense pipe, encapsulating the actual device and sensors
        self.pipe = rs.pipeline()
        # !! This whole function needs to be refactored into the ManagedCamera class !!
        # !! It currently only handles one camera !!

        # Configure image stream(s)
        config = rs.config()
        # if self.device_id:  # This logic moves to ManagedCamera based on its serial
    ...
    def realsense_configure_setting(self, setting_file):
        # !! This function needs refactoring. It should operate on a specific device object !!
        # Find *which* device to configure? Currently uses the old logic.
        device = self.find_device_that_supports_advanced_mode()
        # advnc_mode = rs.rs400_advanced_mode(device) # Moved inside enable
        advnc_mode = self.realsense_enable_advanced_mode(device)
        self.realsense_load_settings_file(advnc_mode, setting_file) # Pass advnc_mode
    ...
    def set_obstacle_distance_params(self):
        # !! This needs to be done per camera, using its specific intrinsics and orientation !!
